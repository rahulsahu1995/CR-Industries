...
export default function WhatsAppButton() {
  return (
    <motion.a
      ...
      whileTap={{ scale: 0.94 }}
      className="group fixed bottom-5 right-5 sm:bottom-6 sm:right-6 z-50 flex items-center gap-2"
    >
      {/* Tooltip — hidden on mobile, slides in on hover */}
      <span className="hidden sm:inline-flex items-center px-3 py-1.5 rounded-full bg-foreground text-background text-xs font-bold shadow-lg shadow-black/20 opacity-0 translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300 pointer-events-none whitespace-nowrap">
        Chat with us
      </span>

      {/* Circle button */}
      <span className="relative flex items-center justify-center w-14 h-14 sm:w-15 sm:h-15 rounded-full shadow-xl shadow-black/25 ring-4 ring-white/40 dark:ring-white/10" style={{ backgroundColor: "#25D366" }}>
        {/* Pulsing halo */}
        <motion.span
          aria-hidden
